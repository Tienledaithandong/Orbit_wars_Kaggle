...
# Create mock module structure for loading checkpoints saved with different module paths
...
# Define mock dataclasses for checkpoint compatibility
@dataclass
...
def fixed_ship_count(src: PlanetState, tgt: PlanetState) -> int:
    # Need target ships + 1 to capture, minimum 3 for early game aggression
...
def agent(obs, config=None):
    ...
    if _policy is None:
        ...
        # Try fixed checkpoint first (compatible with PyTorch 2.6+)
        weight_path = Path(__file__).parent / "weights" / "ckpt_fixed.pt"
        
        # Fallback to original checkpoint paths
