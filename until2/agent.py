# ============================================================
# ORBIT WARS – LEARNING AGENT v8.0
# ============================================================
import math
import time
...
DEFAULT_CFG = {
    ...
    "OPPONENT_AGGRESSION_MEMORY": 8,
    "AGGRESSIVE_OPPONENT_THRESHOLD": 0.35,
    "PASSIVE_OPPONENT_THRESHOLD": 0.15,
    "AGGRESSIVE_DEFENSE_MULT": 1.25,
    "PASSIVE_ATTACK_MARGIN_BONUS": 0.18,

    "PRECISION_REINFORCE_WINDOW": 2,
    "LATE_ELIMINATION_BONUS": 25.0,
    "FLEET_SPEED_SNIPE_BONUS": 0.12,

    "COMET_STAGING_ENABLED": True,
    "COMET_STAGING_MIN_LIFE": 8,
    ...
    "OPP_PREDICT_HORIZON": 22,         # how far ahead we predict enemy moves
    "OPP_PREDICT_TOP_MOVES": 4,        # max predicted fleets per opponent
    "OPP_PREDICT_WEIGHT_IN_DEFENSE": 0.45,  # how much we trust predicted attacks
    "OPP_PREDICT_CONTEST_PENALTY": 0.85,    # how much we devalue a target if opponent likely attacks it
}
...
# ============================================================
class OpponentModel:
    def __init__(self, player, num_opponents, cfg):
        self.player = player
        self.history = defaultdict(list)
        self.aggression_score = defaultdict(float)
        self.last_update = 0
        self.cfg = cfg

    def update(self, world, fleets, step):
        if step == self.last_update:
            return
        self.last_update = step

        for fleet in fleets:
            if fleet.owner == self.player or fleet.owner == -1:
                continue
            opp_planets = [p for p in world.planets if p.owner == fleet.owner]
            if not opp_planets:
                continue
            src_dist = min(dist(fleet.x, fleet.y, p.x, p.y) for p in opp_planets)
            if src_dist < 5.0:
                self.history[fleet.owner].append((step, fleet.ships))

        for opp_id in list(self.history.keys()):
            total_ships = world.owner_strength.get(opp_id, 1)
            recent = sum(s for t, s in self.history[opp_id]
                         if step - t <= self.cfg["OPPONENT_AGGRESSION_MEMORY"])
            self.aggression_score[opp_id] = recent / max(1, total_ships)

    def is_aggressive(self, opp_id):
        return self.aggression_score.get(opp_id, 0.5) > self.cfg["AGGRESSIVE_OPPONENT_THRESHOLD"]

    def is_passive(self, opp_id):
        return self.aggression_score.get(opp_id, 0.5) < self.cfg["PASSIVE_OPPONENT_THRESHOLD"]

    def get_defense_multiplier(self, opp_id):
        if self.is_aggressive(opp_id):
            return self.cfg["AGGRESSIVE_DEFENSE_MULT"]
        return 1.0
...
def orbital_radius(planet):
    return dist(planet.x, planet.y, DEFAULT_CFG["CENTER_X"], DEFAULT_CFG["CENTER_Y"])

def is_static_planet(planet):
    return orbital_radius(planet) + planet.radius >= DEFAULT_CFG["ROTATION_LIMIT"]
...
def fleet_speed(ships):
    ...
    return 1.0 + (DEFAULT_CFG["MAX_SPEED"] - 1.0) * (ratio ** 1.5)
...
def segment_hits_sun(x1, y1, x2, y2, safety=None):
    if safety is None:
        safety = DEFAULT_CFG["SUN_SAFETY"]
    return point_to_segment_distance(DEFAULT_CFG["CENTER_X"], DEFAULT_CFG["CENTER_Y"], x1, y1, x2, y2) < DEFAULT_CFG["SUN_R"] + safety

def launch_point(sx, sy, sr, angle):
    clearance = sr + DEFAULT_CFG["LAUNCH_CLEARANCE"]
...
def actual_path_geometry(sx, sy, sr, tx, ty, tr):
    ...
    hit_distance = max(0.0, dist(sx, sy, tx, ty) - (sr + DEFAULT_CFG["LAUNCH_CLEARANCE"]) - tr)
...
def predict_planet_position(planet, initial_by_id, angular_velocity, turns):
    ...
    r = dist(init.x, init.y, DEFAULT_CFG["CENTER_X"], DEFAULT_CFG["CENTER_Y"])
    if r + init.radius >= DEFAULT_CFG["ROTATION_LIMIT"]:
        return planet.x, planet.y
    cur_ang = math.atan2(planet.y - DEFAULT_CFG["CENTER_Y"], planet.x - DEFAULT_CFG["CENTER_X"])
    new_ang = cur_ang + angular_velocity * turns
    return DEFAULT_CFG["CENTER_X"] + r * math.cos(new_ang), DEFAULT_CFG["CENTER_Y"] + r * math.sin(new_ang)
...
def target_can_move(target, initial_by_id, comet_ids):
    ...
    if init is None:
        return False
    r = dist(init.x, init.y, DEFAULT_CFG["CENTER_X"], DEFAULT_CFG["CENTER_Y"])
    return r + init.radius < DEFAULT_CFG["ROTATION_LIMIT"]

def search_safe_intercept(src, target, ships, initial_by_id, ang_vel, comets, comet_ids, cfg):
    best = None
    best_score = None
    max_turns = min(cfg["HORIZON"], cfg["ROUTE_SEARCH_HORIZON"])
    if target.id in comet_ids:
    ...
    for candidate_turns in range(1, max_turns + 1):
        ...
        if est is None:
            continue
        _, turns = est
        if abs(turns - candidate_turns) > cfg["INTERCEPT_TOLERANCE"]:
            continue
        ...
        if delta > cfg["INTERCEPT_TOLERANCE"]:
            continue
...
def aim_with_prediction(src, target, ships, initial_by_id, ang_vel, comets, comet_ids, cfg):
    ...
    if est is None:
        if not target_can_move(target, initial_by_id, comet_ids):
            return None
        return search_safe_intercept(src, target, ships, initial_by_id, ang_vel, comets, comet_ids, cfg)
    ...
    for _ in range(5):
        ...
        if next_est is None:
            if not target_can_move(target, initial_by_id, comet_ids):
                return None
            return search_safe_intercept(src, target, ships, initial_by_id, ang_vel, comets, comet_ids, cfg)
        if abs(ntx - tx) < 0.3 and abs(nty - ty) < 0.3 and abs(next_est[1] - turns) <= cfg["INTERCEPT_TOLERANCE"]:
...
# World Model
# ============================================================
def fleet_target_planet(fleet, planets, cfg):
    best_planet = None
    best_time = 1e9
    ...
    for planet in planets:
        ...
        turns = hit_d / speed
        if turns <= cfg["HORIZON"] and turns < best_time:
            best_time = turns
            best_planet = planet
...
def build_arrival_ledger(fleets, planets, cfg):
    arrivals_by_planet = {planet.id: [] for planet in planets}
    for fleet in fleets:
        target, eta = fleet_target_planet(fleet, planets, cfg)
        if target is None:
            continue
...
class WorldModel:
    def __init__(self, player, step, planets, fleets, initial_by_id, ang_vel, comets, comet_ids, cfg=None):
        if cfg is None:
            cfg = DEFAULT_CFG
        self.cfg = cfg
        self.player = player
        ...
        self.remaining_steps = max(1, cfg["TOTAL_STEPS"] - step)
        self.is_early = step < cfg["EARLY_TURN_LIMIT"]
        self.is_opening = step < cfg["OPENING_TURN_LIMIT"]
        self.is_late = self.remaining_steps < cfg["LATE_REMAINING_TURNS"]
        self.is_very_late = self.remaining_steps < cfg["VERY_LATE_REMAINING_TURNS"]
        ...
        self.arrivals_by_planet = build_arrival_ledger(fleets, planets, cfg)
        self.base_timeline = {
            planet.id: simulate_planet_timeline(planet, self.arrivals_by_planet[planet.id], player, cfg["HORIZON"])
            for planet in planets
        }
        ...
        self.opp_model = OpponentModel(player, self.num_players - 1, cfg)
        self.opp_model.update(self, fleets, step)

        # Opponent prediction – only if enabled (prediction horizon > 0)
        if self.cfg["OPP_PREDICT_HORIZON"] > 0:
            self.predicted_opponent_moves = self._predict_opponent_fleets()
        else:
            self.predicted_opponent_moves = {}

    def _predict_opponent_fleets(self) -> Dict[int, List[Tuple[int, int, int, int]]]:
        """
        For each opponent, predict their likely fleet launches using a simplified heuristic.
        Returns dict: opp_id -> list of (src_id, target_id, ships, eta)
        """
        horizon = self.cfg["OPP_PREDICT_HORIZON"]
        top_moves = self.cfg["OPP_PREDICT_TOP_MOVES"]
        predictions = {}
        for opp_id, opp_ps in self.opp_planets.items():
            opp_planets = self.planet_by_id.values()  # all planets for simplicity
            # Build a stripped-down policy for this opponent
            # We mimic a simple version of our own scoring
            shots = []
            for src in opp_ps:
                src_avail = int(src.ships)
                if src_avail < 6:
                    continue
                for target in self.planets:
                    if target.id == src.id or target.owner == opp_id:
                        continue
                    # Quick reaction check
                    my_reaction = self.reaction_times(target.id)[0]  # my reaction (actual me)
                    enemy_reaction = self.reaction_times(target.id)[1]  # quick approximation
                    # Skip if too far
                    seeded = self.best_probe_aim(src.id, target.id, src_avail, hints=(int(target.ships) + 1,))
                    if seeded is None:
                        continue
                    _, aim = seeded
                    eta = aim[1]
                    if eta > horizon:
                        continue
                    # Simple value: production * remaining steps
                    value = target.production * max(1, self.remaining_steps - eta)
                    # Only consider if value positive
                    if value <= 0:
                        continue
                    # Rough needed
                    needed = max(1, int(target.ships) + 1)  # very rough
                    score = value / (needed + eta * 0.5)  # simple score
                    shots.append((score, src.id, target.id, needed, eta))
            shots.sort(reverse=True, key=lambda x: x[0])
            # Select top moves
            chosen = []
            used_src = defaultdict(int)
            for score, src_id, tgt_id, needed, eta in shots:
                if len(chosen) >= top_moves:
                    break
                if used_src[src_id] + needed > self.planet_by_id[src_id].ships:
                    continue
                chosen.append((src_id, tgt_id, needed, eta))
                used_src[src_id] += needed
            predictions[opp_id] = chosen
        return predictions
    ...
    def plan_shot(self, src_id, target_id, ships):
        ...
        result = aim_with_prediction(src, target, ships, self.initial_by_id, self.ang_vel, self.comets, self.comet_ids, self.cfg)
    ...
    def probe_ship_candidates(self, src_id, target_id, source_cap, hints=()):
        ...
        values.update({source_cap, max(1, source_cap // 2), max(1, source_cap // 3),
                      min(source_cap, self.cfg["PARTIAL_SOURCE_MIN_SHIPS"]), min(source_cap, target_ships + 1),
                      min(source_cap, target_ships + 2), min(source_cap, target_ships + 4),
    ...
    def hold_status(self, target_id, planned_commitments=None, horizon=None):
        if horizon is None:
        ...
        return {"keep_needed": tl["keep_needed"], "min_owned": tl["min_owned"], "first_enemy": tl["first_enemy"], "fall_turn": tl["fall_turn"], "holds_full": tl["holds_full"]}
    ...
    def min_ships_to_own_by(self, target_id, eval_turn, attacker_owner, arrival_turn=None, planned_commitments=None, extra_arrivals=(), upper_bound=None):
        ...
        normalized_extra = tuple((max(1, int(math.ceil(turns))), owner, int(ships)) for turns, owner, ships in extra_arrivals if ships > 0 and max(1, int(math.ceil(turns))) <= eval_turn)
        cache_key = None
...
# ============================================================
# Strategy Functions (unchanged except use cfg where needed)
# ============================================================
...
def candidate_time_valid(target, turns, world, remaining_buffer):
    ...
    if target.id in world.comet_ids:
        ...
        if turns >= life or turns > world.cfg["COMET_MAX_CHASE_TURNS"]:
            return False
    return True
...
def stacked_enemy_proactive_keep(planet, world):
    ...
    for enemy in world.enemy_planets:
        ...
        _, aim = seeded
        eta = aim[1]
        if eta > world.cfg["MULTI_ENEMY_PROACTIVE_HORIZON"]:
            continue
    ...
    for right in range(len(threats)):
        ...
        while threats[right][0] - threats[left][0] > world.cfg["MULTI_ENEMY_STACK_WINDOW"]:
            running -= threats[left][1]
            left += 1
        best_stacked = max(best_stacked, running)
    return int(best_stacked * world.cfg["MULTI_ENEMY_PROACTIVE_RATIO"])

def swarm_eta_tolerance(options, target, world):
    if len(options) >= 3:
        return world.cfg["THREE_SOURCE_ETA_TOLERANCE"]
    if target.owner not in (-1, world.player):
        return world.cfg["HOSTILE_SWARM_ETA_TOLERANCE"]
    return world.cfg["MULTI_SOURCE_ETA_TOLERANCE"]
    ...
    for target_id, arrivals in world.arrivals_by_planet.items():
        ...
        for i in range(len(enemy_events)):
            ...
            for j in range(i + 1, len(enemy_events)):
                ...
                if owner_a == owner_b:
                    continue
                if abs(eta_a - eta_b) > world.cfg["CRASH_EXPLOIT_ETA_WINDOW"]:
                    break
                if ships_a + ships_b < world.cfg["CRASH_EXPLOIT_MIN_TOTAL_SHIPS"]:
                    continue
                crashes.append({"target_id": target_id, "crash_turn": max(eta_a, eta_b), "owners": (owner_a, owner_b), "ships": (ships_a, ships_b)})
    return crashes
...
def build_policy_state(world, deadline=None):
    ...
    for target_id, features in world.indirect_feature_map.items():
        ...
        indirect_wealth_map[target_id] = friendly * cfg["INDIRECT_FRIENDLY_WEIGHT"] + neutral * cfg["INDIRECT_NEUTRAL_WEIGHT"] + enemy * cfg["INDIRECT_ENEMY_WEIGHT"]
    ...
    for target in world.planets:
        ...
        if target.owner == world.player:
            continue
        my_sources = nearest_sources_to_target(target, world.my_planets, cfg["REACTION_SOURCE_TOP_K_MY"])
        enemy_sources = nearest_sources_to_target(target, world.enemy_planets, cfg["REACTION_SOURCE_TOP_K_ENEMY"])
    ...
    # Incorporate opponent predicted fleets as extra threats
    opponent_threat = defaultdict(int)
    if world.predicted_opponent_moves:
        for opp_id, moves in world.predicted_opponent_moves.items():
            for (src_id, tgt_id, ships, eta) in moves:
                if eta <= cfg["PROACTIVE_DEFENSE_HORIZON"] + 5:
                    opponent_threat[tgt_id] += int(ships * cfg["OPP_PREDICT_WEIGHT_IN_DEFENSE"])

    for planet in world.my_planets:
        if expired():
            break
        exact_keep = world.keep_needed_map.get(planet.id, 0)
        proactive_keep = opponent_threat.get(planet.id, 0)
        # Original proactive keep from nearest enemies
        for enemy in nearest_sources_to_target(planet, world.enemy_planets, cfg["PROACTIVE_ENEMY_TOP_K"]):
            enemy_aim = world.plan_shot(enemy.id, planet.id, max(1, int(enemy.ships)))
            if enemy_aim is None:
                continue
            enemy_eta = enemy_aim[1]
            if enemy_eta > cfg["PROACTIVE_DEFENSE_HORIZON"]:
                continue
            proactive_keep = max(proactive_keep, int(enemy.ships * cfg["PROACTIVE_DEFENSE_RATIO"]))
        proactive_keep = max(proactive_keep, stacked_enemy_proactive_keep(planet, world))

        nearest_enemy = min(world.enemy_planets, key=lambda e: dist(planet.x, planet.y, e.x, e.y), default=None)
        if nearest_enemy:
            mult = world.opp_model.get_defense_multiplier(nearest_enemy.owner)
            proactive_keep = int(proactive_keep * mult)
    ...
    return {"indirect_wealth_map": indirect_wealth_map, "reserve": reserve, "attack_budget": attack_budget, "reaction_time_map": reaction_time_map}

def build_modes(world):
    cfg = world.cfg
    domination = (world.my_total - world.enemy_total) / max(1, world.my_total + world.enemy_total)
    is_behind = domination < cfg["BEHIND_DOMINATION"]
    is_ahead = domination > cfg["AHEAD_DOMINATION"]
    is_dominating = is_ahead or (world.max_enemy_strength > 0 and world.my_total > world.max_enemy_strength * 1.25)
    is_finishing = domination > cfg["FINISHING_DOMINATION"] and world.my_prod > world.enemy_prod * cfg["FINISHING_PROD_RATIO"] and world.step > 100
    attack_margin_mult = 1.0
    if is_ahead:
        attack_margin_mult += cfg["AHEAD_ATTACK_MARGIN_BONUS"]
    if is_behind:
        attack_margin_mult -= cfg["BEHIND_ATTACK_MARGIN_PENALTY"]
    if is_finishing:
        attack_margin_mult += cfg["FINISHING_ATTACK_MARGIN_BONUS"]
    return {"domination": domination, "is_behind": is_behind, "is_ahead": is_ahead, "is_dominating": is_dominating, "is_finishing": is_finishing, "attack_margin_mult": attack_margin_mult}

def is_safe_neutral(target, policy, world):
    if target.owner != -1:
        return False
    my_t, enemy_t = policy_reaction_times(target.id, policy)
    return my_t <= enemy_t - world.cfg["SAFE_NEUTRAL_MARGIN"]

def is_contested_neutral(target, policy, world):
    if target.owner != -1:
        return False
    my_t, enemy_t = policy_reaction_times(target.id, policy)
    return abs(my_t - enemy_t) <= world.cfg["CONTESTED_NEUTRAL_MARGIN"]
...
def opening_filter(target, arrival_turns, needed, src_available, world, policy):
    ...
    if target.production >= cfg["SAFE_OPENING_PROD_THRESHOLD"] and arrival_turns <= cfg["SAFE_OPENING_TURN_LIMIT"] and reaction_gap >= cfg["SAFE_NEUTRAL_MARGIN"]:
        return False
    if world.is_four_player:
        affordable = needed <= max(cfg["PARTIAL_SOURCE_MIN_SHIPS"], int(src_available * cfg["FOUR_PLAYER_ROTATING_SEND_RATIO"]))
        if affordable and arrival_turns <= cfg["FOUR_PLAYER_ROTATING_TURN_LIMIT"] and reaction_gap >= cfg["FOUR_PLAYER_ROTATING_REACTION_GAP"]:
            return False
        return True
    return arrival_turns > cfg["ROTATING_OPENING_MAX_TURNS"] or target.production <= cfg["ROTATING_OPENING_LOW_PROD"]

def elimination_value(target, world, modes):
    cfg = world.cfg
    if target.owner in (-1, world.player):
        return 0.0
    opp_strength = world.owner_strength.get(target.owner, 0)
    opp_prod = world.owner_production.get(target.owner, 0)
    if opp_strength < cfg["WEAK_ENEMY_THRESHOLD"] and opp_prod < world.my_prod * 0.3:
        return cfg["LATE_ELIMINATION_BONUS"]
    return 0.0
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    ...
    value += policy["indirect_wealth_map"][target.id] * turns_profit * cfg["INDIRECT_VALUE_SCALE"]

    if world.is_static(target.id):
        value *= cfg["STATIC_NEUTRAL_VALUE_MULT"] if target.owner == -1 else cfg["STATIC_HOSTILE_VALUE_MULT"]
    else:
        value *= cfg["ROTATING_OPENING_VALUE_MULT"] if world.is_opening else 1.0

    if target.owner not in (-1, world.player):
        base_mult = cfg["OPENING_HOSTILE_TARGET_VALUE_MULT"] if world.is_opening else cfg["HOSTILE_TARGET_VALUE_MULT"]
        opp_priority = world.opponent_priority(target.owner)
        value *= base_mult * opp_priority

    if target.owner == -1:
        if is_safe_neutral(target, policy, world):
            value *= cfg["SAFE_NEUTRAL_VALUE_MULT"]
        elif is_contested_neutral(target, policy, world):
            value *= cfg["CONTESTED_NEUTRAL_VALUE_MULT"]
        if world.is_early:
            value *= cfg["EARLY_NEUTRAL_VALUE_MULT"]

    if target.id in world.comet_ids:
        value *= cfg["COMET_VALUE_MULT"]

    if mission == "snipe":
        value *= cfg["SNIPE_VALUE_MULT"]
    elif mission == "swarm":
        value *= cfg["SWARM_VALUE_MULT"]
    elif mission == "reinforce":
        value *= cfg["REINFORCE_VALUE_MULT"]
    elif mission == "crash_exploit":
        value *= cfg["CRASH_EXPLOIT_VALUE_MULT"]

    if world.is_late:
        value += max(0, target.ships) * cfg["LATE_IMMEDIATE_SHIP_VALUE"]
        if target.owner not in (-1, world.player):
            enemy_strength = world.owner_strength.get(target.owner, 0)
            if enemy_strength <= cfg["WEAK_ENEMY_THRESHOLD"]:
                value += cfg["ELIMINATION_BONUS"]

    value += elimination_value(target, world, modes)

    if modes["is_finishing"] and target.owner not in (-1, world.player):
        value *= cfg["FINISHING_HOSTILE_VALUE_MULT"]
    if modes["is_behind"] and target.owner == -1 and not world.is_static(target.id):
        value *= cfg["BEHIND_ROTATING_NEUTRAL_VALUE_MULT"]
    if modes["is_behind"] and target.owner == -1 and is_safe_neutral(target, policy, world):
        value *= 1.08
    if modes["is_dominating"] and target.owner == -1 and is_contested_neutral(target, policy, world):
        value *= 0.92

    # Apply opponent contest penalty
    if world.predicted_opponent_moves:
        for opp_id, moves in world.predicted_opponent_moves.items():
            for (_, tgt_id, ships, eta) in moves:
                if tgt_id == target.id and abs(eta - arrival_turns) <= 3:
                    value *= cfg["OPP_PREDICT_CONTEST_PENALTY"]
                    break
...
def reinforce_value(target, hold_until, world, policy):
    ...
    value = target.production * saved_turns + max(0, target.ships) * cfg["DEFENSE_SHIP_VALUE"]
    if world.enemy_planets and nearest_distance_to_set(target.x, target.y, world.enemy_planets) < 22:
        value *= cfg["DEFENSE_FRONTIER_SCORE_MULT"]
    value += policy["indirect_wealth_map"][target.id] * saved_turns * cfg["INDIRECT_VALUE_SCALE"] * 0.35
    return value * cfg["REINFORCE_VALUE_MULT"]
...
def preferred_send(target, base_needed, arrival_turns, src_available, world, modes, policy):
    ...
    margin = 0
    if target.owner == -1:
        margin += min(cfg["NEUTRAL_MARGIN_CAP"], cfg["NEUTRAL_MARGIN_BASE"] + target.production * cfg["NEUTRAL_MARGIN_PROD_WEIGHT"])
    else:
        margin += min(cfg["HOSTILE_MARGIN_CAP"], cfg["HOSTILE_MARGIN_BASE"] + target.production * cfg["HOSTILE_MARGIN_PROD_WEIGHT"])
        reinforce_est = 0
        for ep in world.opp_planets.get(target.owner, []):
            ...
            if ep_aim is None:
                continue
            ep_eta = ep_aim[1]
            if ep_eta <= arrival_turns + cfg["HOSTILE_REINFORCE_HORIZON"]:
                sendable = max(0, int(ep.ships) - 3)
                reinforce_est += sendable
        reinforce_margin = min(cfg["HOSTILE_REINFORCE_CAP"], int(reinforce_est * cfg["HOSTILE_REINFORCE_RATIO"]))
        margin += reinforce_margin
    if world.is_static(target.id):
        margin += cfg["STATIC_TARGET_MARGIN"]
    if is_contested_neutral(target, policy, world):
        margin += cfg["CONTESTED_TARGET_MARGIN"]
    if world.is_four_player:
        margin += cfg["FOUR_PLAYER_TARGET_MARGIN"]
    if arrival_turns > cfg["LONG_TRAVEL_MARGIN_START"]:
        margin += min(cfg["LONG_TRAVEL_MARGIN_CAP"], arrival_turns // cfg["LONG_TRAVEL_MARGIN_DIVISOR"])
    if target.id in world.comet_ids:
        margin = max(0, margin - cfg["COMET_MARGIN_RELIEF"])
    if modes["is_finishing"] and target.owner not in (-1, world.player):
        margin += cfg["FINISHING_HOSTILE_SEND_BONUS"]
...
def apply_score_modifiers(base_score, target, mission, world):
    ...
    score = base_score
    if world.is_static(target.id):
        score *= cfg["STATIC_TARGET_SCORE_MULT"]
    if world.is_early and target.owner == -1 and world.is_static(target.id):
        score *= cfg["EARLY_STATIC_NEUTRAL_SCORE_MULT"]
    if world.is_four_player and target.owner == -1 and not world.is_static(target.id):
        score *= cfg["FOUR_PLAYER_ROTATING_NEUTRAL_SCORE_MULT"]
    if len(world.static_neutral_planets) >= cfg["DENSE_STATIC_NEUTRAL_COUNT"] and target.owner == -1 and not world.is_static(target.id):
        score *= cfg["DENSE_ROTATING_NEUTRAL_SCORE_MULT"]
    if mission == "snipe":
        score *= cfg["SNIPE_SCORE_MULT"]
    elif mission == "swarm":
        score *= cfg["SWARM_SCORE_MULT"]
    elif mission == "crash_exploit":
        score *= cfg["CRASH_EXPLOIT_SCORE_MULT"]
    return score
...
def settle_plan(src, target, src_cap, send_guess, world, planned_commitments, modes, policy, mission="capture", eval_turn_fn=None, anchor_turn=None, anchor_tolerance=None, max_iter=4):
    ...
    def evaluate(send):
        ...
        if mission in ("snipe", "crash_exploit"):
            ...
            desired = min(src_cap, max(need, need + cfg["DEFENSE_SEND_MARGIN_BASE"] + target.production * cfg["DEFENSE_SEND_MARGIN_PROD_WEIGHT"]))
        else:
    ...
    for _ in range(max_iter):
        ...
        if next_send in tested:
            ...
            break
        current_send = next_send
    candidate_sends = sorted([send for send in tested_order if tested.get(send) is not None], key=lambda send: (0 if mission != "snipe" or anchor_turn is None else abs(tested[send][1] - anchor_turn), abs(send - seed_hint), tested[send][1], send))
    seen = set()
...
def settle_reinforce_plan(src, target, src_cap, send_guess, world, planned_commitments, hold_until, max_arrival_turn, max_iter=4):
    ...
    def evaluate(send):
        ...
        if need <= 0 or need > src_cap:
            tested[send] = None
            return None
        desired = min(src_cap, need + cfg["REINFORCE_SAFETY_MARGIN"])
...
def precision_reinforce_plan(src, target, src_cap, fall_turn, world, planned_commitments):
    cfg = world.cfg
    best = None
    best_score = None
    for send in world.probe_ship_candidates(src.id, target.id, src_cap, hints=(target.ships + 5,)):
        aim = world.plan_shot(src.id, target.id, send)
        if aim is None:
            continue
        angle, turns, _, _ = aim
        if abs(turns - fall_turn) > cfg["PRECISION_REINFORCE_WINDOW"]:
            continue
        score = (abs(turns - fall_turn), -turns, -send)
        if best is None or score < best_score:
            best_score = score
            best = (angle, turns, send)
    return best

def speed_snipe_plan(src, target, src_cap, desired_turn, world):
    best = None
    best_score = None
    for mult in (1.0, 1.2, 1.5, 2.0):
        send = min(src_cap, int((target.ships + 1) * mult))
        if send < 1:
            continue
        aim = world.plan_shot(src.id, target.id, send)
        if aim is None:
            continue
        angle, turns, _, _ = aim
        if turns > desired_turn + 1:
            continue
        arrival_score = (abs(turns - desired_turn), -send)
        if best is None or arrival_score < best_score:
            best_score = arrival_score
            best = (angle, turns, send)
    return best

def comet_staging_candidate(world):
    cfg = world.cfg
    candidates = []
    for comet_id in world.comet_ids:
        life = world.comet_life(comet_id)
        if life < cfg["COMET_STAGING_MIN_LIFE"]:
            continue
        comet = world.planet_by_id[comet_id]
        if comet.owner != -1:
            continue
        my_avg_dist = sum(dist(p.x, p.y, comet.x, comet.y) for p in world.my_planets) / max(1, len(world.my_planets))
        enemy_min_dist = min((dist(e.x, e.y, comet.x, comet.y) for e in world.enemy_planets), default=999)
        if enemy_min_dist < my_avg_dist * 0.8:
            candidates.append(comet)
    return candidates
...
def build_snipe_mission(src, target, src_available, world, planned_commitments, modes, policy):
    ...
    for enemy_eta in enemy_etas[:3]:
        ...
        if target.id in world.comet_ids:
            life = world.comet_life(target.id)
            if sync_turn >= life or sync_turn > cfg["COMET_MAX_CHASE_TURNS"]:
                continue
        ...
        speed_plan = speed_snipe_plan(src, target, src_available, turns, world)
        if speed_plan:
            speed_angle, speed_turns, speed_send = speed_plan
            if speed_turns < turns:
                angle, turns, send_pref = speed_angle, speed_turns, speed_send

        value = target_value(target, sync_turn, "snipe", world, modes, policy)
        if value <= 0:
            continue
        score = apply_score_modifiers(value / (send_pref + sync_turn * cfg["SNIPE_COST_TURN_WEIGHT"] + 1.0), target, "snipe", world)
...
def build_rescue_missions(world, policy, planned_commitments, modes):
    ...
    for target in world.my_planets:
        ...
        if fall_turn is None or fall_turn > cfg["DEFENSE_LOOKAHEAD_TURNS"]:
            continue
        for src in world.my_planets:
            if src.id == target.id:
                continue
            src_available = policy["attack_budget"].get(src.id, 0)
            if src_available < cfg["PARTIAL_SOURCE_MIN_SHIPS"]:
                continue
            seeded = world.best_probe_aim(src.id, target.id, src_available, hints=(target.production + cfg["DEFENSE_SEND_MARGIN_BASE"] + 2,), max_turn=fall_turn)
            if seeded is None:
                continue
            ...
            if world.enemy_planets and nearest_distance_to_set(target.x, target.y, world.enemy_planets) < 22:
                value *= cfg["DEFENSE_FRONTIER_SCORE_MULT"]
            score = value / (send_pref + turns * cfg["DEFENSE_COST_TURN_WEIGHT"] + 1.0)
...
def build_recapture_missions(world, policy, planned_commitments, modes):
    ...
    for target in world.my_planets:
        ...
        for src in world.my_planets:
            ...
            if src_available < cfg["PARTIAL_SOURCE_MIN_SHIPS"]:
                continue
            seeded = world.best_probe_aim(src.id, target.id, src_available, hints=(target.production + cfg["DEFENSE_SEND_MARGIN_BASE"] + 2,), min_turn=fall_turn + 1, max_turn=fall_turn + cfg["RECAPTURE_LOOKAHEAD_TURNS"])
            if seeded is None:
                continue
            probe, probe_aim = seeded
            ...
            if plan is None:
                continue
            angle, turns, _, need, send_pref = plan
            if turns <= fall_turn or turns - fall_turn > cfg["RECAPTURE_LOOKAHEAD_TURNS"]:
                continue
            saved_turns = max(1, world.remaining_steps - turns)
            value = cfg["RECAPTURE_PRODUCTION_WEIGHT"] * target.production * saved_turns + cfg["RECAPTURE_IMMEDIATE_WEIGHT"] * max(0, target.ships)
            if world.enemy_planets and nearest_distance_to_set(target.x, target.y, world.enemy_planets) < 22:
                value *= cfg["RECAPTURE_FRONTIER_MULT"]
            value *= cfg["RECAPTURE_VALUE_MULT"]
            score = value / (send_pref + turns * cfg["RECAPTURE_COST_TURN_WEIGHT"] + 1.0)
...
def build_reinforce_missions(world, policy, planned_commitments, modes, inventory_left_fn):
    ...
    if not cfg["REINFORCE_ENABLED"]:
        return []
    missions = []
    if world.remaining_steps < cfg["REINFORCE_MIN_FUTURE_TURNS"]:
        return missions
    for target in world.my_planets:
        fall_turn = world.fall_turn_map.get(target.id)
        if fall_turn is None:
            continue
        if target.production < cfg["REINFORCE_MIN_PRODUCTION"]:
            continue
        hold_until = min(cfg["HORIZON"], fall_turn + cfg["REINFORCE_HOLD_LOOKAHEAD"])
        max_arrival_turn = min(fall_turn, cfg["REINFORCE_MAX_TRAVEL_TURNS"])
        for src in world.my_planets:
            if src.id == target.id:
                continue
            budget = inventory_left_fn(src.id)
            source_cap = min(budget, int(src.ships * cfg["REINFORCE_MAX_SOURCE_FRACTION"]))
            if source_cap < cfg["PARTIAL_SOURCE_MIN_SHIPS"]:
                continue

            precision = precision_reinforce_plan(src, target, source_cap, fall_turn, world, planned_commitments)
            if precision:
                angle, turns, send = precision
                value = reinforce_value(target, hold_until, world, policy)
                score = value / (send + turns * cfg["REINFORCE_COST_TURN_WEIGHT"] + 1.0)
                option = ShotOption(score=score, src_id=src.id, target_id=target.id, angle=angle, turns=turns, needed=send, send_cap=send, mission="reinforce", anchor_turn=hold_until)
                missions.append(Mission(kind="reinforce", score=score, target_id=target.id, turns=fall_turn, options=[option]))
                continue

            seeded = world.best_probe_aim(src.id, target.id, source_cap, hints=(target.production + cfg["REINFORCE_SAFETY_MARGIN"] + 2,), max_turn=max_arrival_turn)
            if seeded is None:
                continue
            probe, _ = seeded
            ...
            angle, turns, _, need, send_pref = plan
            value = reinforce_value(target, hold_until, world, policy)
            score = value / (send_pref + turns * cfg["REINFORCE_COST_TURN_WEIGHT"] + 1.0)
...
def build_crash_exploit_missions(world, policy, planned_commitments, modes):
    cfg = world.cfg
    if not cfg["CRASH_EXPLOIT_ENABLED"] or not world.is_four_player:
        return []
    missions = []
    for crash in detect_enemy_crashes(world):
        ...
        if target.owner == world.player:
            continue
        desired_arrival = crash["crash_turn"] + cfg["CRASH_EXPLOIT_POST_CRASH_DELAY"]
        for src in world.my_planets:
            src_available = policy["attack_budget"].get(src.id, 0)
            if src_available < cfg["PARTIAL_SOURCE_MIN_SHIPS"]:
                continue
            seeded = world.best_probe_aim(src.id, target.id, src_available, hints=(12, int(target.ships) + 1), anchor_turn=desired_arrival, max_anchor_diff=cfg["CRASH_EXPLOIT_ETA_WINDOW"])
            if seeded is None:
                continue
            probe, _ = seeded
            plan = settle_plan(src, target, src_available, probe, world, planned_commitments, modes, policy, mission="crash_exploit", eval_turn_fn=lambda turns, desired_arrival=desired_arrival: max(turns, desired_arrival), anchor_turn=desired_arrival, anchor_tolerance=cfg["CRASH_EXPLOIT_ETA_WINDOW"])
            if plan is None:
                continue
            angle, turns, _, need, send_pref = plan
            if not candidate_time_valid(target, turns, world, cfg["LATE_CAPTURE_BUFFER"]):
                continue
            value = target_value(target, turns, "crash_exploit", world, modes, policy)
            if value <= 0:
                continue
            score = apply_score_modifiers(value / (send_pref + turns * cfg["SNIPE_COST_TURN_WEIGHT"] + 1.0), target, "crash_exploit", world)
...
def build_comet_staging_missions(world, policy, planned_commitments, modes):
    cfg = world.cfg
    if not cfg["COMET_STAGING_ENABLED"] or world.is_late:
        return []
    missions = []
    for comet in comet_staging_candidate(world):
        for src in world.my_planets:
            src_avail = policy["attack_budget"].get(src.id, 0)
            if src_avail < 10:
                continue
            aim = world.plan_shot(src.id, comet.id, min(src_avail, comet.ships + 5))
            if aim is None:
                continue
            angle, turns, _, _ = aim
            if turns > world.comet_life(comet.id) - 3:
                continue
            value = comet.production * (world.comet_life(comet.id) - turns) * 1.5
            score = value / (10 + turns * 0.5)
            option = ShotOption(score, src.id, comet.id, angle, turns, comet.ships + 2, comet.ships + 5, mission="staging")
            missions.append(Mission("staging", score, comet.id, turns, [option]))
    return missions
...
def plan_moves(world, deadline=None):
    ...
    def allow_heavy_phase():
        return time_left() > cfg["HEAVY_PHASE_MIN_TIME"] and len(world.planets) <= cfg["HEAVY_ROUTE_PLANET_LIMIT"]

    def allow_optional_phase():
        return time_left() > cfg["OPTIONAL_PHASE_MIN_TIME"]
    ...
    def compute_live_doomed():
        doomed = set()
        for planet in world.my_planets:
            status = world.hold_status(planet.id, planned_commitments=planned_commitments, horizon=cfg["DOOMED_EVAC_TURN_LIMIT"])
            if not status["holds_full"] and status["fall_turn"] is not None and status["fall_turn"] <= cfg["DOOMED_EVAC_TURN_LIMIT"] and source_inventory_left(planet.id) >= cfg["DOOMED_MIN_SHIPS"]:
                doomed.add(planet.id)
        return doomed

    def time_filters_pass(target, turns, needed, src_cap):
        if not candidate_time_valid(target, turns, world, cfg["VERY_LATE_CAPTURE_BUFFER"] if world.is_very_late else cfg["LATE_CAPTURE_BUFFER"]):
            return False
    ...
    if allow_heavy_phase():
        ...
        missions.extend(build_comet_staging_missions(world, policy, planned_commitments, modes))
    ...
    for src in world.my_planets:
        ...
        for target in world.planets:
            ...
            if not candidate_time_valid(target, rough_turns, world, cfg["VERY_LATE_CAPTURE_BUFFER"] if world.is_very_late else cfg["LATE_CAPTURE_BUFFER"]):
                continue
            ...
            if partial_send_cap >= cfg["PARTIAL_SOURCE_MIN_SHIPS"]:
                ...
                if partial_seed is not None:
                    ...
                    if time_filters_pass(target, p_turns, global_needed, src_available):
                        ...
                        if partial_value > 0:
                            partial_score = apply_score_modifiers(partial_value / (partial_send_cap + p_turns * cfg["ATTACK_COST_TURN_WEIGHT"] + 1.0), target, "swarm", world)
            ...
            if global_needed <= src_available:
                ...
                if value <= 0:
                    continue
                score = apply_score_modifiers(value / (send_cap + turns * cfg["ATTACK_COST_TURN_WEIGHT"] + 1.0), target, "capture", world)
    ...
    for target_id, options in source_options_by_target.items():
        ...
        top_options = sorted(options, key=lambda item: -item.score)[:cfg["MULTI_SOURCE_TOP_K"]]
        for i in range(len(top_options)):
            for j in range(i + 1, len(top_options)):
                ...
                if value <= 0:
                    continue
                pair_score = apply_score_modifiers(value / (need + joint_turn * cfg["ATTACK_COST_TURN_WEIGHT"] + 1.0), target, "swarm", world)
                pair_score *= cfg["MULTI_SOURCE_PLAN_PENALTY"]
                missions.append(Mission(kind="swarm", score=pair_score, target_id=target_id, turns=joint_turn, options=[first, second]))
        if cfg["THREE_SOURCE_SWARM_ENABLED"] and allow_heavy_phase() and target.owner not in (-1, world.player) and int(target.ships) >= cfg["THREE_SOURCE_MIN_TARGET_SHIPS"] and len(top_options) >= 3:
            for i in range(len(top_options)):
                for j in range(i + 1, len(top_options)):
                    for k in range(j + 1, len(top_options)):
                        ...
                        if value <= 0:
                            continue
                        trio_score = apply_score_modifiers(value / (need + joint_turn * cfg["ATTACK_COST_TURN_WEIGHT"] + 1.0), target, "swarm", world)
                        trio_score *= cfg["THREE_SOURCE_PLAN_PENALTY"]
    ...
    for mission in missions:
        ...
        if mission.kind in ("single", "snipe", "rescue", "recapture", "reinforce", "crash_exploit", "staging"):
            option = mission.options[0]
            src = world.planet_by_id[option.src_id]
            if mission.kind == "reinforce":
                left = min(source_inventory_left(option.src_id), int(src.ships * cfg["REINFORCE_MAX_SOURCE_FRACTION"]))
            else:
            ...
            if mission.kind == "reinforce":
                ...
                plan = settle_plan(src, target, left, min(left, option.send_cap), world, planned_commitments, modes, policy, mission="crash_exploit", eval_turn_fn=lambda turns, desired_arrival=option.anchor_turn: max(turns, desired_arrival), anchor_turn=option.anchor_turn, anchor_tolerance=cfg["CRASH_EXPLOIT_ETA_WINDOW"])
            elif mission.kind == "staging":
    ...
    if not world.is_very_late and allow_optional_phase():
        for src in world.my_planets:
            ...
            if src_left < cfg["FOLLOWUP_MIN_SHIPS"]:
                continue
            best = None
            for target in world.planets:
                if expired():
                ...
                if target.id == src.id or target.owner == world.player:
                    continue
                if target.id in world.comet_ids and target.production <= cfg["LOW_VALUE_COMET_PRODUCTION"]:
                    continue
                ...
                if world.is_late and est_turns > world.remaining_steps - cfg["LATE_CAPTURE_BUFFER"]:
                    continue
                ...
                if world.is_late and turns > world.remaining_steps - cfg["LATE_CAPTURE_BUFFER"]:
                    continue
                if final_send < need:
                    continue
                value = target_value(target, turns, "capture", world, modes, policy)
                if value <= 0:
                    continue
                score = apply_score_modifiers(value / (final_send + turns * cfg["ATTACK_COST_TURN_WEIGHT"] + 1.0), target, "capture", world)
    ...
    if (world.enemy_planets or world.neutral_planets) and len(world.my_planets) > 1 and not world.is_late and allow_optional_phase():
        ...
        if safe_fronts:
            ...
            send_ratio = cfg["REAR_SEND_RATIO_FOUR_PLAYER"] if world.is_four_player else cfg["REAR_SEND_RATIO_TWO_PLAYER"]
            if modes["is_finishing"]:
                send_ratio = max(send_ratio, cfg["REAR_SEND_RATIO_FOUR_PLAYER"])
            for rear in sorted(world.my_planets, key=lambda planet: -frontier_distance[planet.id]):
                ...
                if rear.id == front_anchor.id or rear.id in live_doomed:
                    continue
                if source_attack_left(rear.id) < cfg["REAR_SOURCE_MIN_SHIPS"]:
                    continue
                if frontier_distance[rear.id] < frontier_distance[front_anchor.id] * cfg["REAR_DISTANCE_RATIO"]:
                    continue
                stage_candidates = [planet for planet in safe_fronts if planet.id != rear.id and frontier_distance[planet.id] < frontier_distance[rear.id] * cfg["REAR_STAGE_PROGRESS"]]
                if stage_candidates:
                ...
                if send < cfg["REAR_SEND_MIN_SHIPS"]:
                    continue
                aim = world.plan_shot(rear.id, front.id, send)
                if aim is None:
                    continue
                angle, turns, _, _ = aim
                if turns > cfg["REAR_MAX_TRAVEL_TURNS"]:
                    continue
...
def build_world(obs, cfg=None):
    ...
    return WorldModel(player=player, step=step, planets=planets, fleets=fleets, initial_by_id=initial_by_id, ang_vel=ang_vel, comets=comets, comet_ids=comet_ids, cfg=cfg)

def agent(obs, config=None):
    start_time = time.perf_counter()
    # Merge any provided weights into DEFAULT_CFG
    cfg = DEFAULT_CFG.copy()
    if config is not None:
        weights = _read(config, "weights", None)
        if weights is not None:
            cfg.update(weights)
    world = build_world(obs, cfg)
    if not world.my_planets:
        return []
    act_timeout = _read(config, "actTimeout", 1.0) if config is not None else 1.0
    soft_budget = min(cfg["SOFT_ACT_DEADLINE"], max(0.55, act_timeout * 0.82))
