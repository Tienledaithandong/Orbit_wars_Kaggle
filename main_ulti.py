# ============================================================
# ORBIT WARS – SOTA AGENT v7.5 (1300+ Target)
# ============================================================
# Features: Timeline Simulation, Opponent Modeling, Precision
#           Reinforcement, Elimination Priority, Comet Staging
# ============================================================
...
class OpponentModel:
    """Tracks opponent aggression levels for adaptive defense."""
    ...
    def update(self, world, fleets, step):
        ...
        for opp_id in list(self.history.keys()):
            ...
            recent = sum(s for t, s in self.history[opp_id] 
                        if step - t <= OPPONENT_AGGRESSION_MEMORY)
            self.aggression_score[opp_id] = recent / max(1, total_ships)

    def is_aggressive(self, opp_id):
        return self.aggression_score.get(opp_id, 0.5) > AGGRESSIVE_OPPONENT_THRESHOLD

    def is_passive(self, opp_id):
        return self.aggression_score.get(opp_id, 0.5) < PASSIVE_OPPONENT_THRESHOLD

    def get_defense_multiplier(self, opp_id):
        if self.is_aggressive(opp_id):
            return AGGRESSIVE_DEFENSE_MULT
        return 1.0
...
class WorldModel:
    ...
    def probe_ship_candidates(self, src_id, target_id, source_cap, hints=()):
        ...
        values.update({source_cap, max(1, source_cap // 2), max(1, source_cap // 3),
                      min(source_cap, PARTIAL_SOURCE_MIN_SHIPS), min(source_cap, target_ships + 1),
...
def build_policy_state(world, deadline=None):
    ...
    for planet in world.my_planets:
        ...
        # Adaptive defense based on opponent aggression
...
def elimination_value(target, world, modes):
    """Extra value for eliminating a weak opponent."""
...
def precision_reinforce_plan(src, target, src_cap, fall_turn, world, planned_commitments):
    """Find a fleet size that arrives within PRECISION_REINFORCE_WINDOW of fall_turn."""
    best = None
    best_score = None
    for send in world.probe_ship_candidates(src.id, target.id, src_cap, hints=(target.ships + 5,)):
        ...
        if aim is None:
            continue
        angle, turns, _, _ = aim
        if abs(turns - fall_turn) > PRECISION_REINFORCE_WINDOW:
            continue
...
def speed_snipe_plan(src, target, src_cap, desired_turn, world):
    """Try larger fleets to arrive earlier for snipes."""
    best = None
    best_score = None
...
def comet_staging_candidate(world):
    """Find comets that can serve as forward staging points."""
    candidates = []
...
def build_reinforce_missions(world, policy, planned_commitments, modes, inventory_left_fn):
    ...
    for target in world.my_planets:
        ...
        for src in world.my_planets:
            ...
            # Try precision reinforcement first
            ...
            # Fallback to standard reinforcement
...
def build_comet_staging_missions(world, policy, planned_commitments, modes):
    """Capture comets as forward staging points."""
...
def build_world(obs):
    ...
    return WorldModel(player=player, step=step, planets=planets, fleets=fleets, initial_by_id=initial_by_id, ang_vel=ang_vel, comets=comets, comet_ids=comet_ids)
