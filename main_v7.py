"""
Orbit Wars – v7 Champion
Wave Sync + Over‑send + Dynamic Garrison + Coordinated Strikes + Backline Reinforcement + Evacuation + Endgame Consolidation + ROI Scoring
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Coordinated Multi‑planet Strike
# ─────────────────────────────────────────────────────────────────────────────
...
def agent(obs):
    try:
        ...
        print(f"[v7 ERROR] {e}", file=sys.stderr)
...
def _agent_impl(obs):
    ...
    def target_score(t):
        ...
        if t.owner != -1:
            prod *= 1.8
        return (prod * 100) / (d + t.ships * 0.2 + 1)
    ...
    # ── Phase 1: Wave Sync on top 3 high‑value targets ──────────────────────
    if not is_late:
        enemy_planets.sort(key=target_score, reverse=True)
        for tgt in enemy_planets[:3]:
    ...
    # ── Phase 5: Endgame consolidation ───────────────────────────────────────
    if remaining_steps < 35:
        # Sort allied planets by ships, send unused ships from weak to strong
        best_planet = max(my_planets, key=lambda p: p.ships, default=None)
        if best_planet:
            for src in my_planets:
                if src.id == best_planet.id: continue
                avail = available.get(src.id, 0) - used[src.id]
                if avail > 10:
                    res = _intercept(src, best_planet, avail, init_map, ang_vel, comet_data, False)
                    if res[0] is not None and not _crosses_sun(src.x, src.y, res[0], res[1]):
                        tx, ty, tt = res
                        if step + tt < TOTAL_TURNS - 1: # Arrive before game ends
                            ang = math.atan2(ty - src.y, tx - src.x)
                            moves.append([src.id, ang, avail])
                            used[src.id] += avail
