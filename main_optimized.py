# ============================================================
# ORBIT WARS – OPTIMIZED SOTA AGENT v8.0
# ============================================================
# Optimizations: Streamlined physics, cached computations,
#                efficient data structures, reduced allocations
# ============================================================
...
# ============================================================
# Configuration - Consolidated & Tuned
# ============================================================
...
# Opponent Modeling (simplified for speed)
...
def actual_path_geometry(sx, sy, sr, tx, ty, tr):
    ...
    hit_distance = max(0.0, math.hypot(sx - tx, sy - ty) - (sr + LAUNCH_CLEARANCE) - tr)
    return angle, start_x, start_y, start_x + math.cos(angle) * hit_distance, start_y + math.sin(angle) * hit_distance, hit_distance
...
def travel_time(sx, sy, sr, tx, ty, tr, ships):
    ...
    return 10**9 if est is None else est[1]
...
def target_can_move(target, initial_by_id, comet_ids):
    ...
    if init is None:
        return False
    return math.hypot(init.x - CENTER_X, init.y - CENTER_Y) + init.radius < ROTATION_LIMIT
...
def search_safe_intercept(src, target, ships, initial_by_id, ang_vel, comets, comet_ids):
    ...
    for candidate_turns in range(1, max_turns + 1):
        ...
        if confirm is None or abs(confirm[1] - actual_turns) > INTERCEPT_TOLERANCE:
            continue
...
def resolve_arrival_event(owner, garrison, arrivals):
    ...
    if len(sorted_players) > 1 and sorted_players[1][1] == top_ships:
    ...
    return (survivor_owner, -garrison) if garrison < 0 else (owner, garrison)

def normalize_arrivals(arrivals, horizon):
    events = [(max(1, int(math.ceil(turns))), owner, int(ships)) 
              for turns, owner, ships in arrivals 
              if ships > 0 and int(math.ceil(turns)) <= horizon]
...
def simulate_planet_timeline(planet, arrivals, player, horizon):
    ...
    for turn in range(1, horizon + 1):
        ...
        if group:
            if owner == player and first_enemy is None and any(item[1] not in (-1, player) for item in group):
                first_enemy = turn
            prev_owner = owner
    ...
    return {
        "owner_at": owner_at, "ships_at": ships_at, "keep_needed": keep_needed,
...
def state_at_timeline(timeline, arrival_turn):
    turn = min(max(0, int(math.ceil(arrival_turn))), timeline["horizon"])
    return timeline["owner_at"].get(turn, timeline["owner_at"][timeline["horizon"]]), \
           max(0.0, timeline["ships_at"].get(turn, timeline["ships_at"][timeline["horizon"]]))
...
def nearest_distance_to_set(px, py, planets):
    if not planets:
        return 10**9
    return min(math.hypot(px - p.x, py - p.y) for p in planets)
...
def indirect_features(planet, planets, player):
    ...
    for other in planets:
        if other.id == planet.id:
            continue
        d = math.hypot(planet.x - other.x, planet.y - other.y)
        if d < 1:
            continue
...
class OpponentModel:
    __slots__ = ('player', 'history', 'aggression_score', 'last_update')
    
    def __init__(self, player, num_opponents):
        self.player = player
    ...
    def update(self, world, fleets, step):
        ...
        for fleet in fleets:
            if fleet.owner in (self.player, -1):
                continue
            opp_planets = [p for p in world.planets if p.owner == fleet.owner]
            if not opp_planets:
                continue
            src_dist = min(math.hypot(fleet.x - p.x, fleet.y - p.y) for p in opp_planets)
            if src_dist < 5.0:
        ...
        for opp_id in list(self.history.keys()):
            ...
            recent = sum(s for t, s in self.history[opp_id] if step - t <= OPPONENT_AGGRESSION_MEMORY)
            self.aggression_score[opp_id] = recent / max(1, total_ships)

    def get_defense_multiplier(self, opp_id):
        return AGGRESSIVE_DEFENSE_MULT if self.aggression_score.get(opp_id, 0.5) > AGGRESSIVE_OPPONENT_THRESHOLD else 1.0

class WorldModel:
    __slots__ = ('player', 'step', 'planets', 'fleets', 'initial_by_id', 'ang_vel', 'comets', 'comet_ids',
                 'planet_by_id', 'my_planets', 'enemy_planets', 'neutral_planets', 'static_neutral_planets',
                 'num_players', 'remaining_steps', 'is_early', 'is_opening', 'is_late', 'is_very_late', 'is_four_player',
                 'owner_strength', 'owner_production', 'my_total', 'enemy_total', 'max_enemy_strength', 'my_prod', 'enemy_prod',
                 'arrivals_by_planet', 'base_timeline', 'keep_needed_map', 'min_owned_map', 'first_enemy_map',
                 'fall_turn_map', 'holds_full_map', 'indirect_feature_map', 'shot_cache', 'probe_candidate_cache',
                 'best_probe_cache', 'reaction_cache', 'exact_need_cache', 'total_visible_ships', 'total_production',
                 'opp_planets', 'opp_min_dist', 'opp_model')
    ...
    def __init__(self, player, step, planets, fleets, initial_by_id, ang_vel, comets, comet_ids):
        ...
        for opp_id, opp_ps in self.opp_planets.items():
            min_d = min(math.hypot(mp.x - ep.x, mp.y - ep.y) for mp in self.my_planets for ep in opp_ps) if self.my_planets else 1e9
            self.opp_min_dist[opp_id] = min_d

        self.opp_model = OpponentModel(player, self.num_players - 1)
    ...
    def opponent_priority(self, opp_id):
        ...
        return max(0.5, proximity_score + strength_share * (0.25 if similar > 1 else 0.05))
    ...
    def plan_shot(self, src_id, target_id, ships):
        ...
        if key not in self.shot_cache:
            src, target = self.planet_by_id[src_id], self.planet_by_id[target_id]
            self.shot_cache[key] = aim_with_prediction(src, target, ships, self.initial_by_id, self.ang_vel, self.comets, self.comet_ids)
        return self.shot_cache[key]

    def probe_ship_candidates(self, src_id, target_id, source_cap, hints=()):
        cache_key = (src_id, target_id, max(1, int(source_cap)), tuple(int(math.ceil(h)) for h in hints if h))
        ...
        for i in (1, 2, 4, 8):
            values.add(min(source_cap, target_ships + i))
        for hint in (int(math.ceil(h)) for h in hints if h):
            base = max(1, min(source_cap, hint))
            for d in (-2, -1, 0, 1, 2):
                values.add(max(1, min(source_cap, base + d)))
    ...
    def best_probe_aim(self, src_id, target_id, source_cap, hints=(), min_turn=None, max_turn=None, anchor_turn=None, max_anchor_diff=None):
        ...
        for ships in self.probe_ship_candidates(src_id, target_id, source_cap, hints=hints):
            ...
            if aim is None:
                continue
            angle, turns, _, _ = aim
            if (min_turn and turns < min_turn) or (max_turn and turns > max_turn):
                continue
            if anchor_turn and max_anchor_diff and abs(turns - anchor_turn) > max_anchor_diff:
                continue
            
            key = (abs(turns - anchor_turn), turns, ships) if anchor_turn else (turns, ships)
            if best_key is None or key < best_key:
                best_key = key
                best = (ships, (angle, turns, _, _))
    ...
    def reaction_times(self, target_id):
        ...
        target = self.planet_by_id[target_id]
        my_t = min((a[1] for s in self.my_planets if (a := self.best_probe_aim(s.id, target.id, max(1, int(s.ships)))) is not None), default=10**9)
        enemy_t = min((a[1] for s in self.enemy_planets if (a := self.best_probe_aim(s.id, target.id, max(1, int(s.ships)))) is not None), default=10**9)
    ...
    def projected_timeline(self, target_id, horizon, planned_commitments=None, extra_arrivals=()):
        ...
        return simulate_planet_timeline(self.planet_by_id[target_id], arrivals, self.player, horizon)

    def hold_status(self, target_id, planned_commitments=None, horizon=HORIZON):
        tl = self.projected_timeline(target_id, horizon, planned_commitments=planned_commitments) if planned_commitments.get(target_id) else self.base_timeline[target_id]
        return {"keep_needed": tl["keep_needed"], "min_owned": tl["min_owned"], "first_enemy": tl["first_enemy"], 
                "fall_turn": tl["fall_turn"], "holds_full": tl["holds_full"]}
    ...
    def min_ships_to_own_by(self, target_id, eval_turn, attacker_owner, arrival_turn=None, planned_commitments=None, extra_arrivals=(), upper_bound=None):
        ...
        if arrival_turn > eval_turn:
            return max(1, int(upper_bound)) + 1 if upper_bound else self._ownership_search_cap(eval_turn) + 1

        normalized_extra = tuple((max(1, int(math.ceil(turns))), owner, int(ships)) 
                                 for turns, owner, ships in extra_arrivals 
                                 if ships > 0 and max(1, int(math.ceil(turns))) <= eval_turn)
        
        cache_key = (target_id, eval_turn, attacker_owner) if arrival_turn == eval_turn and not planned_commitments.get(target_id) and not normalized_extra else None
        if cache_key and cache_key in self.exact_need_cache:
            return self.exact_need_cache[cache_key]
        ...
        def owns_at(ships):
            owner_after, _ = self.projected_state(target_id, eval_turn, planned_commitments=planned_commitments, 
                                                   extra_arrivals=normalized_extra + ((arrival_turn, attacker_owner, int(ships)),))
            return owner_after == attacker_owner

        hi = max(1, int(upper_bound)) if upper_bound else max(1, int(math.ceil(ships_before)) + 1)
    ...
    def min_ships_to_own_at(self, target_id, arrival_turn, attacker_owner, planned_commitments=None, extra_arrivals=(), upper_bound=None):
        return self.min_ships_to_own_by(target_id, arrival_turn, attacker_owner, arrival_turn=arrival_turn, 
    ...
    def reinforcement_needed_to_hold_until(self, planet_id, arrival_turn, hold_until, planned_commitments=None, upper_bound=None):
        ...
        if target.owner != self.player:
            return self.min_ships_to_own_by(planet_id, hold_until, self.player, arrival_turn=arrival_turn, 
                                           planned_commitments=planned_commitments, upper_bound=upper_bound)

        def holds_with_reinforcement(ships):
            timeline = self.projected_timeline(planet_id, hold_until, planned_commitments=planned_commitments, 
                                               extra_arrivals=((arrival_turn, self.player, int(ships)),))
            return all(timeline["owner_at"].get(turn) == self.player for turn in range(arrival_turn, hold_until + 1))

        hi = max(1, int(upper_bound)) if upper_bound else 1
...
def min_legal_reaction_time(target, sources, world):
    return min((a[1] for s in sources if (a := world.best_probe_aim(s.id, target.id, max(1, int(s.ships)))) is not None), default=10**9)
...
def candidate_time_valid(target, turns, world, remaining_buffer):
    ...
    if target.id in world.comet_ids:
        ...
        return turns < life and turns <= COMET_MAX_CHASE_TURNS
    return True

def stacked_enemy_proactive_keep(planet, world):
    threats = [(a[1], int(e.ships)) for e in world.enemy_planets if (a := world.best_probe_aim(e.id, planet.id, max(1, int(e.ships)))) is not None and a[1] <= MULTI_ENEMY_PROACTIVE_HORIZON]
    if not threats:
        return 0
    threats.sort()
...
def swarm_eta_tolerance(options, target, world):
    ...
    return HOSTILE_SWARM_ETA_TOLERANCE if target.owner not in (-1, world.player) else MULTI_SOURCE_ETA_TOLERANCE

def detect_enemy_crashes(world):
    crashes = []
    for target_id, arrivals in world.arrivals_by_planet.items():
        enemy_events = sorted([(int(math.ceil(eta)), owner, int(ships)) for eta, owner, ships in arrivals 
...
def build_policy_state(world, deadline=None):
    ...
    indirect_wealth_map = {tid: f * INDIRECT_FRIENDLY_WEIGHT + n * INDIRECT_NEUTRAL_WEIGHT + e * INDIRECT_ENEMY_WEIGHT for tid, (f, n, e) in world.indirect_feature_map.items()}
    reserve = {}
    attack_budget = {}
    reaction_time_map = {}
    ...
    for target in world.planets:
        ...
        reaction_time_map[target.id] = (min_legal_reaction_time(target, my_sources, world), 
                                        min_legal_reaction_time(target, enemy_sources, world))
    ...
    for planet in world.my_planets:
        ...
        for enemy in nearest_sources_to_target(planet, world.enemy_planets, PROACTIVE_ENEMY_TOP_K):
            ...
            if enemy_aim and enemy_aim[1] <= PROACTIVE_DEFENSE_HORIZON:
        ...
        nearest_enemy = min(world.enemy_planets, key=lambda e: math.hypot(planet.x - e.x, planet.y - e.y), default=None)
        if nearest_enemy:
            proactive_keep = int(proactive_keep * world.opp_model.get_defense_multiplier(nearest_enemy.owner))
...
def build_modes(world):
    ...
    return {"domination": domination, "is_behind": is_behind, "is_ahead": is_ahead, 
            "is_dominating": is_dominating, "is_finishing": is_finishing, "attack_margin_mult": attack_margin_mult}
...
def opening_filter(target, arrival_turns, needed, src_available, world, policy):
    if not world.is_opening or target.owner != -1 or target.id in world.comet_ids or world.is_static(target.id):
        return False
...
def elimination_value(target, world, modes):
    ...
    if opp_strength < WEAK_ENEMY_THRESHOLD and opp_prod < world.my_prod * 0.3:
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    ...
    if world.is_static(target.id):
        ...
        value *= 0.9  # ROTATING_OPENING_VALUE_MULT

    if target.owner not in (-1, world.player):
        value *= (1.45 if world.is_opening else HOSTILE_TARGET_VALUE_MULT) * world.opponent_priority(target.owner)
    ...
    if target.owner == -1:
        ...
        if world.is_early:
            value *= 1.2  # EARLY_NEUTRAL_VALUE_MULT

    if target.id in world.comet_ids:
        value *= COMET_VALUE_MULT

    mission_mults = {"snipe": SNIPE_VALUE_MULT, "swarm": SWARM_VALUE_MULT, "reinforce": REINFORCE_VALUE_MULT, "crash_exploit": 1.18}
    if mission in mission_mults:
        value *= mission_mults[mission]

    if world.is_late:
        value += max(0, target.ships) * LATE_IMMEDIATE_SHIP_VALUE
        if target.owner not in (-1, world.player) and world.owner_strength.get(target.owner, 0) <= WEAK_ENEMY_THRESHOLD:
    ...
    if modes["is_finishing"] and target.owner not in (-1, world.player):
        value *= 1.15
    if modes["is_behind"]:
        if target.owner == -1 and not world.is_static(target.id):
            value *= 0.92
        elif target.owner == -1 and is_safe_neutral(target, policy):
            value *= 1.08
...
def preferred_send(target, base_needed, arrival_turns, src_available, world, modes, policy):
    ...
    if target.owner == -1:
        ...
        reinforce_est = sum(max(0, int(ep.ships) - 3) for ep in world.opp_planets.get(target.owner, []) 
                          if ep.id != target.id and (ep_aim := world.plan_shot(ep.id, target.id, max(1, int(ep.ships)))) and ep_aim[1] <= arrival_turns + HOSTILE_REINFORCE_HORIZON)
...
def apply_score_modifiers(base_score, target, mission, world):
    ...
    mission_mults = {"snipe": SNIPE_SCORE_MULT, "swarm": SWARM_SCORE_MULT, "crash_exploit": CRASH_EXPLOIT_SCORE_MULT}
    if mission in mission_mults:
        score *= mission_mults[mission]
    return score
...
def settle_plan(src, target, src_cap, send_guess, world, planned_commitments, modes, policy, mission="capture", eval_turn_fn=None, anchor_turn=None, anchor_tolerance=None, max_iter=4):
    ...
    def evaluate(send):
        ...
        desired = need if mission in ("snipe", "crash_exploit") else min(src_cap, max(need, preferred_send(target, need, turns, src_cap, world, modes, policy)))
        if mission == "rescue":
    ...
    for seed in initial_candidates:
        ...
        if result and (anchor_turn is None or anchor_tolerance is None or abs(result[1] - anchor_turn) <= anchor_tolerance):
            current_send = seed
            break
    ...
    candidate_sends = sorted([send for send in tested_order if tested.get(send)], 
    ...
    for send in candidate_sends:
        ...
        if actual_send >= need and (anchor_turn is None or anchor_tolerance is None or abs(turns - anchor_turn) <= anchor_tolerance):
            if mission != "rescue" or turns <= eval_turn:
                return angle, turns, eval_turn, need, actual_send
    return None
...
def settle_reinforce_plan(src, target, src_cap, send_guess, world, planned_commitments, hold_until, max_arrival_turn, max_iter=4):
    ...
    def evaluate(send):
        ...
        if send in tested:
            return tested[send]
        aim = world.plan_shot(src.id, target.id, send)
        if aim is None or aim[1] > max_arrival_turn:
            tested[send] = None
            return None
        angle, turns, _, _ = aim
    ...
    for send in sorted([s for s in tested_order if tested.get(s)], key=lambda send: (abs(send - seed_hint), tested[send][1], send)):
        result = tested.get(send)
...
def precision_reinforce_plan(src, target, src_cap, fall_turn, world, planned_commitments):
    ...
    for send in world.probe_ship_candidates(src.id, target.id, src_cap, hints=(target.ships + 5,)):
        aim = world.plan_shot(src.id, target.id, send)
        if aim is None or abs(aim[1] - fall_turn) > 2:  # PRECISION_REINFORCE_WINDOW
            continue
        score = (abs(aim[1] - fall_turn), -aim[1], -send)
        if best is None or score < best_score:
            best_score = score
            best = (aim[0], aim[1], send)
    return best
...
def speed_snipe_plan(src, target, src_cap, desired_turn, world):
    ...
    for mult in (1.0, 1.2, 1.5, 2.0):
        ...
        if send < 1:
            continue
        aim = world.plan_shot(src.id, target.id, send)
        if aim is None or aim[1] > desired_turn + 1:
            continue
        arrival_score = (abs(aim[1] - desired_turn), -send)
...
def comet_staging_candidate(world):
    ...
    for comet_id in world.comet_ids:
        ...
        if life < COMET_STAGING_MIN_LIFE:
            continue
        comet = world.planet_by_id[comet_id]
        if comet.owner != -1:
            continue
        my_avg_dist = sum(math.hypot(p.x - comet.x, p.y - comet.y) for p in world.my_planets) / max(1, len(world.my_planets))
        enemy_min_dist = min((math.hypot(e.x - comet.x, e.y - comet.y) for e in world.enemy_planets), default=999)
...
def build_snipe_mission(src, target, src_available, world, planned_commitments, modes, policy):
    ...
    enemy_etas = sorted({int(math.ceil(eta)) for eta, owner, ships in world.arrivals_by_planet.get(target.id, []) 
                         if owner not in (-1, world.player) and ships > 0})
    if not enemy_etas:
        return None
    ...
    for enemy_eta in enemy_etas[:3]:
        ...
        plan = settle_plan(src, target, src_available, probe, world, planned_commitments, modes, policy, mission="snipe", 
                          eval_turn_fn=lambda turns, enemy_eta=enemy_eta: max(turns, enemy_eta), anchor_turn=enemy_eta)
        if plan is None:
            continue
        ...
        if speed_plan and speed_plan[1] < turns:
            angle, turns, send_pref = speed_plan
...
def build_rescue_missions(world, policy, planned_commitments, modes):
    ...
    for target in world.my_planets:
        ...
        for src in world.my_planets:
            ...
            plan = settle_plan(src, target, src_available, probe, world, planned_commitments, modes, policy, mission="rescue", 
                              eval_turn_fn=lambda _turns, fall_turn=fall_turn: fall_turn, anchor_turn=fall_turn)
            if plan is None:
                continue
    ...
    for target in world.my_planets:
        ...
        for src in world.my_planets:
            ...
            if precision:
                ...
                score = value / (send + turns * REINFORCE_COST_TURN_WEIGHT + 1.0)
    ...
    for crash in detect_enemy_crashes(world):
        ...
        for src in world.my_planets:
            ...
            if seeded is None:
                continue
            probe, _ = seeded
            plan = settle_plan(src, target, src_available, probe, world, planned_commitments, modes, policy, mission="crash_exploit", 
            ...
            if plan is None or not candidate_time_valid(target, plan[1], world, LATE_CAPTURE_BUFFER):
                continue
            angle, turns, _, need, send_pref = plan
...
def build_comet_staging_missions(world, policy, planned_commitments, modes):
    if not COMET_STAGING_ENABLED or world.is_late:
        return []
    ...
    for comet in comet_staging_candidate(world):
        for src in world.my_planets:
            ...
            if aim is None or aim[1] > world.comet_life(comet.id) - 3:
                continue
            angle, turns, _, _ = aim
...
def plan_moves(world, deadline=None):
    ...
    def time_left():
        return 10**9 if deadline is None else deadline - time.perf_counter()
    ...
    def compute_live_doomed():
        ...
        for planet in world.my_planets:
            ...
            if not status["holds_full"] and status["fall_turn"] and status["fall_turn"] <= DOOMED_EVAC_TURN_LIMIT and source_inventory_left(planet.id) >= DOOMED_MIN_SHIPS:
                doomed.add(planet.id)
        return doomed

    def time_filters_pass(target, turns, needed, src_cap):
        return candidate_time_valid(target, turns, world, VERY_LATE_CAPTURE_BUFFER if world.is_very_late else LATE_CAPTURE_BUFFER) and not opening_filter(target, turns, needed, src_cap, world, policy)
    ...
    for src in world.my_planets:
        ...
        for target in world.planets:
            ...
            if global_needed <= 0 or opening_filter(target, rough_turns, global_needed, src_available, world, policy):
                continue
    ...
    for target_id, options in source_options_by_target.items():
        ...
        for i in range(len(top_options)):
            for j in range(i + 1, len(top_options)):
                ...
                if first.src_id == second.src_id or abs(first.turns - second.turns) > swarm_eta_tolerance((first, second), target, world):
                    continue
            ...
            for i in range(len(top_options)):
                for j in range(i + 1, len(top_options)):
                    for k in range(j + 1, len(top_options)):
                        ...
                        if max(turns) - min(turns) > swarm_eta_tolerance(tuple(trio), target, world):
                            continue
                        joint_turn = max(turns)
                        total_cap = sum(o.send_cap for o in trio)
                        need = world.min_ships_to_own_at(target_id, joint_turn, world.player, planned_commitments=planned_commitments, upper_bound=total_cap)
                        if need <= 0 or total_cap < need or any(trio[a].send_cap + trio[b].send_cap >= need for a in range(3) for b in range(a + 1, 3)):
                            continue
                        value = target_value(target, joint_turn, "swarm", world, modes, policy)
                        if value > 0:
    ...
    for mission in missions:
        ...
        if mission.kind in ("single", "snipe", "rescue", "recapture", "reinforce", "crash_exploit", "staging"):
            ...
            left = min(source_inventory_left(option.src_id), int(src.ships * REINFORCE_MAX_SOURCE_FRACTION)) if mission.kind == "reinforce" else source_attack_left(option.src_id)
            if left <= 0:
                continue
        ...
        for option, _ in ordered:
            ...
            if fixed_aim is None:
                reaimed = []
                break
            reaimed.append((option.src_id, fixed_aim[0], fixed_aim[1], send))
        
        if not reaimed or max(t[2] for t in reaimed) - min(t[2] for t in reaimed) > swarm_eta_tolerance(mission.options, target, world):
            continue
        
        actual_joint_turn = max(t[2] for t in reaimed)
        owner_after, _ = world.projected_state(target.id, actual_joint_turn, planned_commitments=planned_commitments, extra_arrivals=[(t[2], world.player, t[3]) for t in reaimed])
    ...
    if not world.is_very_late and allow_optional_phase():
        for src in world.my_planets:
            ...
            best = None
            for target in world.planets:
                if expired() or target.id == src.id or target.owner == world.player:
                    continue
                ...
                if rough_needed <= 0 or rough_needed > src_left or opening_filter(target, est_turns, rough_needed, src_left, world, policy):
                    continue
                ...
                if world.is_late and turns > world.remaining_steps - LATE_CAPTURE_BUFFER or final_send < need:
                    continue
                value = target_value(target, turns, "capture", world, modes, policy)
                if value <= 0:
                    continue
    ...
    if live_doomed:
        ...
        frontier_distance = {p.id: nearest_distance_to_set(p.x, p.y, frontier_targets) for p in world.my_planets} if frontier_targets else {p.id: 10**9 for p in world.my_planets}
        for planet in world.my_planets:
            ...
            best_capture = None
            for target in world.planets:
                if expired() or target.id == planet.id or target.owner == world.player:
                    continue
    ...
    if (world.enemy_planets or world.neutral_planets) and len(world.my_planets) > 1 and not world.is_late and allow_optional_phase():
        live_doomed = compute_live_doomed()
        frontier_targets = world.enemy_planets or world.static_neutral_planets or world.neutral_planets
        frontier_distance = {p.id: nearest_distance_to_set(p.x, p.y, frontier_targets) for p in world.my_planets} if frontier_targets else {}
        ...
        if safe_fronts:
            ...
            for rear in sorted(world.my_planets, key=lambda p: -frontier_distance[p.id]):
                if expired() or rear.id == front_anchor.id or rear.id in live_doomed or source_attack_left(rear.id) < REAR_SOURCE_MIN_SHIPS:
                    continue
                if frontier_distance[rear.id] < frontier_distance[front_anchor.id] * REAR_DISTANCE_RATIO:
                    continue
                stage_candidates = [p for p in safe_fronts if p.id != rear.id and frontier_distance[p.id] < frontier_distance[rear.id] * REAR_STAGE_PROGRESS]
                if stage_candidates:
                    ...
                    front = min(remaining_fronts, key=lambda p: planet_distance(p, objective)) if remaining_fronts else None
                if front and front.id != rear.id:
...
def build_world(obs):
    ...
    return WorldModel(player=player, step=step, planets=planets, fleets=fleets, initial_by_id=initial_by_id, 
                     ang_vel=ang_vel, comets=comets, comet_ids=comet_ids)
